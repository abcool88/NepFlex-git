    ...
    order = (object, orderByFields) => {
        return Array.from(object).sort((item1: any, item2: any) => {
            ...
            orderByFields.forEach(element => {
                if (result === 0) {
                    if (this.isNullEmptyOrUndefined(item1[element]) || this.isNullEmptyOrUndefined(item2[element])) {
                        if (this.isNullEmptyOrUndefined(item1[element]) && !this.isNullEmptyOrUndefined(item2[element])) {
                            result = 1;
                        } else if (!this.isNullEmptyOrUndefined(item1[element]) && !this.isNullEmptyOrUndefined(item2[element])){
                            result = -1;
                        }
                    } else if (item1[element] instanceof Date) {
                        if (item1[element].getTime() < item2[element].getTime()) { result = -1; }
                        if (item1[element].getTime() > item2[element].getTime()) { result = 1; }
                    } else {
                        if (item1[element].toString().toLowerCase() < item2[element].toString().toLowerCase()) { result = -1; }
                        if (item1[element].toString().toLowerCase() > item2[element].toString().toLowerCase()) { result = 1; }
                    }
                }
            });
